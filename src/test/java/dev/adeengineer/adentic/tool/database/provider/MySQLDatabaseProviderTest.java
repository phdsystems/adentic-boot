...
/**
 * Tests for MySQLDatabaseProvider (stub implementation).
 *
 * <p>Since MySQL provider is currently a stub, these tests verify stub behavior. These tests will
 * need to be updated when real MySQL implementation is added.
 */
@DisplayName("MySQLDatabaseProvider Tests (Stub)")
class MySQLDatabaseProviderTest {

  private MySQLDatabaseProvider provider;

  @BeforeEach
  void setUp() {
    DatabaseConfig config = DatabaseConfig.mysql();
    provider = new MySQLDatabaseProvider(config);
  }
  ...
  class ConstructorAndInitializationTests {
    ...
    @Test
    @DisplayName("Should initialize with MySQL configuration")
    void shouldInitializeWithMySqlConfiguration() {
      // Given
      DatabaseConfig config = DatabaseConfig.mysql("localhost", 3306, "testdb", "root", "password");

      // When
      MySQLDatabaseProvider mysqlProvider = new MySQLDatabaseProvider(config);

      // Then
      assertNotNull(mysqlProvider);
      assertEquals("mysql", mysqlProvider.getProviderName());
      assertEquals(DatabaseType.SQL, mysqlProvider.getDatabaseType());
      assertFalse(mysqlProvider.isConnected());
    }
    ...
    void shouldHaveCorrectProviderMetadata() {
      // Then
      assertEquals("mysql", provider.getProviderName());
      assertEquals(DatabaseType.SQL, provider.getDatabaseType());
    }

    @Test
    @DisplayName("Should initialize with default MySQL configuration")
    void shouldInitializeWithDefaultMySqlConfiguration() {
      // Given
      DatabaseConfig config = DatabaseConfig.mysql();
      ...
      // Then
      assertNotNull(mysqlProvider);
      assertNotNull(mysqlProvider.getConnectionInfo());
      assertEquals("localhost", mysqlProvider.getConnectionInfo().getHost());
      assertEquals(3306, mysqlProvider.getConnectionInfo().getPort());
      assertEquals("mysql", mysqlProvider.getConnectionInfo().getDatabase());
    }
  }
